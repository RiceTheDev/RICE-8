...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
	...
	else
	{
		...
		if (e > 0)
		{
			...
			// --- BEGIN: Auto-pairing logic ---
			if (!mOverwrite && !IsReadOnly()) {
				char open = buf[0];
				char close = 0;
				switch (open) {
					case '(': close = ')'; break;
				...
				if (close) {
					line.insert(line.begin() + cindex, Glyph(close, PaletteIndex::Default));
					// Move cursor back before the closing char
